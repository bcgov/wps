...
async def get_combustible_area(session: AsyncSession):
    ...
    ```python
    from common.db.crud.auto_spatial_advisory import get_combustible_area
    ...
    logger.info(
        "%f delta count before and after hfi area + zone/area intersection query", delta
    )
    return all_combustible


async def get_all_hfi_thresholds(
    session: AsyncSession,
) -> List[HfiClassificationThreshold]:
    """
...
async def get_zone_ids_in_centre(session: AsyncSession, fire_centre_name: str):
    logger.info(
        f"retrieving fire zones within {fire_centre_name} from advisory_shapes table"
    )
...
async def get_fuel_type_stats_in_advisory_area(
    ...
    stmt = (
        ...
        .join_from(
            AdvisoryFuelStats,
            SFMSFuelType,
            AdvisoryFuelStats.fuel_type == SFMSFuelType.id,
        )
        .filter(
...
async def get_high_hfi_fuel_types_for_shape(
    ...
    """
    logger.info(
        "starting fuel types/high hfi/zone intersection query for fire zone %s",
        str(shape_id),
    )
    perf_start = perf_counter()
    ...
    stmt = (
        ...
        .join_from(
            ClassifiedHfi, FuelType, ClassifiedHfi.geom.ST_Intersects(FuelType.geom)
        )
        .where(
    ...
    delta = perf_end - perf_start
    logger.info(
        "%f delta count before and after fuel types/high hfi/zone intersection query",
        delta,
    )
    return result.all()
...
async def get_run_datetimes(
    ...
    stmt = (
        ...
        .where(
            RunParameters.run_type == run_type.value, RunParameters.for_date == for_date
        )
        .distinct()
...
async def calculate_high_hfi_areas(
    ...
    stmt = (
        select(
            ...
            func.sum(ClassifiedHfi.geom.ST_Intersection(Shape.geom).ST_Area()).label(
                "area"
            ),
        )
...
async def get_provincial_rollup(
    ...
    run_parameter_id = await get_run_parameters_id(
        session, run_type, run_datetime, for_date
    )
    stmt = (
        select(
...
async def save_all_critical_hours(
    session: AsyncSession, critical_hours: List[CriticalHours]
):
