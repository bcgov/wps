...
class PlanningArea(Base):
    ...
    id = Column(
        Integer,
        Sequence("planning_areas_id_seq"),
        primary_key=True,
        nullable=False,
        index=True,
    )
    name = Column(String, nullable=False, index=True)
    fire_centre_id = Column(
        Integer, ForeignKey(FIRE_CENTRES_ID), nullable=False, index=True
    )
...
class PlanningWeatherStation(Base):
    ...
    fuel_type_id = Column(
        Integer, ForeignKey("fuel_types.id"), nullable=False, index=True
    )
    planning_area_id = Column(
        Integer, ForeignKey("planning_areas.id"), nullable=False, index=True
    )
...
class HFIReady(Base):
    ...
    hfi_request_id = Column(
        Integer, ForeignKey(HFI_REQUEST_ID), nullable=False, index=True
    )
    planning_area_id = Column(
        Integer, ForeignKey(PLANNING_AREAS_ID), nullable=False, index=True
    )
