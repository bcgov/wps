...
async def test_c4_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert response.json()['stations'][0]['fuel_type'] == 'C4'
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 14124.210, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 10.312, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.868, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 6.862, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 24.338, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 4.176, abs_tol=0.001)
