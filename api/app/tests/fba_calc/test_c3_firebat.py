...
async def test_c3_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert response.json()['stations'][0]['fuel_type'] == 'C3'
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 2911.249, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 2.754, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.0, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 3.115, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 1.307, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 0.233, abs_tol=0.001)
