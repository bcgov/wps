...
async def test_m4_30deadfir_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 5605.412, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 4.905, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.409, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 4.323, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 3.078, abs_tol=0.1)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 0.367, abs_tol=0.001)
