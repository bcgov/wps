...
async def test_c1_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert response.json()['stations'][0]['fuel_type'] == 'S3'
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 56731.481, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 6.607, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.0, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 13.752, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 7.622, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 1.360, abs_tol=0.001)

    assert response.json()['stations'][0]['fire_type'] == 'SUR'
    assert response.json()['stations'][0]['critical_hours_hfi_4000'] == {'start': 12.0, 'end': 23.0}
