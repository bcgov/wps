...
async def test_c1_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 55905.064, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 6.510, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.0, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 13.651, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 7.398, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 1.320, abs_tol=0.001)
