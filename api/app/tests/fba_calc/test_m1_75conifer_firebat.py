...
async def test_m1_75conifer_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 8767.92, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 8.275, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.694, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 5.406, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 14.512, abs_tol=0.1)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 2.338, abs_tol=0.001)
