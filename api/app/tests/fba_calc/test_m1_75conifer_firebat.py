...
async def test_m1_75conifer_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 8685.398, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 8.204, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.689, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 5.380, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 14.211, abs_tol=0.1)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 2.283, abs_tol=0.001)
