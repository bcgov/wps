...
async def test_m3_60deadfir_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 24454.709, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 19.495, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.979, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 9.029, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 95.492, abs_tol=0.1)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 16.681, abs_tol=0.001)

    assert response.json()['stations'][0]['fire_type'] == 'CC'
    assert response.json()['stations'][0]['critical_hours_hfi_4000'] == {"start": 11.0, "end": 2.0}
    assert response.json()['stations'][0]['critical_hours_hfi_10000'] == {"start": 13.0, "end": 22.0}
