...
async def test_c5_request_response(anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
    ...
    assert response.json()['stations'][0]['fuel_type'] == 'C5'
    ...
    assert math.isclose(response.json()['stations'][0]['head_fire_intensity'], 861.111, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['rate_of_spread'], 0.919, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['percentage_crown_fraction_burned'], 0.0, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['flame_length'], 1.695, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['sixty_minute_fire_size'], 0.135, abs_tol=0.001)
    assert math.isclose(response.json()['stations'][0]['thirty_minute_fire_size'], 0.0241, abs_tol=0.001)
