...
def rate_of_spread(ffmc: float,  # excel column: C29
    ...
    no_wind_ISI = 0.208 * (91.9 * math.exp(-0.1386 * (147.2 * (101 - ffmc) / (59.5 + ffmc))) * (1 +
                           math.pow((147.2 * (101 - ffmc) / (59.5 + ffmc)), 5.31) / (4.93 * math.pow(10, 7)))) * math.exp(0.05038 * 0)
    ...
    isi_adjusted_for_slope_and_wind = 0.208 * (91.9 * math.exp(-0.1386 * (147.2 * (101 - ffmc) / (59.5 + ffmc))) * (1 + math.pow(
        (147.2 * (101 - ffmc) / (59.5 + ffmc)), 5.31) / (4.93 * math.pow(10, 7)))) * math.exp(0.05038 * net_effective_wind_speed)

    # Slope and Wind adjusted RSI
    # C52: =(45*(1-EXP(-0.0305*C51))^2)*C25
    slope_and_wind_adjusted_RSI = (
        45 * math.pow((1 - math.exp(-0.0305 * isi_adjusted_for_slope_and_wind)), 2)) * calculated_curing_factor
