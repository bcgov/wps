...
def get_zone_wind_stats_for_source_id(
    ...
    for zone_wind_stats in zone_wind_stats:
        ...
        all_zone_wind_stats.append(
            AdvisoryMinWindStats(
                threshold=hfi_threshold,
                min_wind_speed=zone_wind_stats.min_wind_speed if zone_wind_stats else None,
            )
        )
...
def build_zone_stats(
    zone_source_ids: Iterable[str],
    hfi_fuel_type_ids_by_zone: dict[int, list],
    hfi_thresholds_by_id: dict[int, HfiThreshold],
    fuel_types: Iterable[DBSFMSFuelType],
    for_date: date,
    zone_wind_stats_by_source_id: dict[int, list],
) -> dict[int, FireZoneHFIStats]:
    all_zone_data: dict[int, FireZoneHFIStats] = {}

    for zone_source_id in zone_source_ids:
        hfi_fuel_type_ids_for_zone = hfi_fuel_type_ids_by_zone.get(zone_source_id, [])
        zone_fuel_stats = []
        for (
            critical_hour_start,
            critical_hour_end,
