...
class WeatherStationModelPrediction(Base):
    ...
    # Cumulative precipitation for model run adjusted for bias.
    bias_adjusted_apcp_sfc_0 = Column(Float, nullable=True)
    ...
    def __str__(self):
        return ('{self.station_code} {self.prediction_timestamp} {self.tmp_tgl_2} {self.bias_adjusted_temperature} '
                ...
                '{self.bias_adjusted_wind_speed} {self.apcp_sfc_0} {self.delta_precip} {self.precip_24h} {self.bias_adjusted_precip_24h}').format(self=self)
...
class MoreCast2MaterializedView(Base):
    ...
    bias_adjusted_apcp_sfc_0 = Column(Float, nullable=False)
    ...
    prediction_model_run_timestamp_id = Column(Integer, nullable=False)
    prediction_timestamp = Column(TZTimeStamp, nullable=False, index=True)
