...
class FuelTypeAreasJob:
    ...
    async def calculate_fuel_type_areas_per_zone(self):
        ...
        async with get_async_write_session_scope() as session:
            fuel_raster_key = await get_fuel_layer_key(RasterKeyAddresser(), session)
            ...
            for zone in zones:
                ...
                for advisory_shape_id, fuel_type_id, fuel_area in fuel_type_area_data:
                    self._save_fuel_type_area(session, advisory_shape_id, sfms_fuel_types_dict[fuel_type_id], fuel_area)
        fuel_raster_ds = None
