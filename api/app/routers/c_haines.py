...
async def _get_most_recent_model_run(model: ModelEnum, data_type: ObjectTypeEnum) -> datetime:
    ...
    async with get_client() as (client, bucket):
        ...
        async def get_most_recent(result, depth):
            ...
            if depth == 3:
                return object_name
            return await get_most_recent(await client.list_objects_v2(Bucket=bucket, Prefix=object_name, Delimiter="/"), depth + 1)

        format_string = "kml" if data_type == ObjectTypeEnum.KML else "json"
        most_recent = await get_most_recent(await client.list_objects_v2(Bucket=bucket, Prefix=f"c-haines-polygons/{format_string}/{model.value}/", Delimiter="/"), 0)
...
async def get_c_haines_model_run(model: ModelEnum, model_run_timestamp: datetime = None, response_format: ObjectTypeEnum = ObjectTypeEnum.GEOJSON):
    """Return geojson/kml polygons for c-haines"""
    logger.info("/c-haines/%s/predictions?model_run_timestamp=%s&response_format=%s", model, model_run_timestamp, response_format)
    ...
    response = StreamingResponse(fetch_model_run_kml_streamer(model, model_run_timestamp), headers=headers, media_type=kml_media_type)
    return response


@router.get("/{model}/prediction")
async def get_c_haines_model_run_prediction(model: ModelEnum, model_run_timestamp: datetime, prediction_timestamp: datetime, response_format: ObjectTypeEnum = ObjectTypeEnum.GEOJSON):
    """Return geojson/kml polygons for c-haines"""
    logger.info("/c-haines/%s/prediction?model_run_timestamp=%s&prediction_timestamp=%s&response_format=%s", model.value, model_run_timestamp, prediction_timestamp, response_format)

    async with get_client() as (client, bucket):
        key = generate_full_object_store_path(model, model_run_timestamp, prediction_timestamp, response_format)
        response = await client.generate_presigned_url("get_object", Params={"Bucket": bucket, "Key": key})
        return RedirectResponse(url=response)


@router.get("/model-runs")
async def get_model_runs(model_run_timestamp: datetime = None, response_format: ObjectTypeEnum = ObjectTypeEnum.GEOJSON):
...
async def get_kml_network_link():
    ...
    headers = {"Content-Type": kml_media_type, "Content-Disposition": "inline;filename=c-haines-network-link.kml"}
