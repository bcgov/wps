...
class FWIKeyFetcher:
    ...
    def determine_last_fwi_date(self, datetime: datetime, model_run_hour: int):
        ...
        if model_run_hour == 0:  # if the model run hour is 00 (17:00 PDT), we should have today's actuals
            day_of_last_actual = local_datetime.date()

        return day_of_last_actual

    def get_previous_dmc_key(self, date: datetime, model_run_hour: int):
        day_of_last_actual = self.determine_last_fwi_date(date, model_run_hour)

        dmc_key = os.path.join(self.sfms_upload_prefix, f"{day_of_last_actual}/dmc{day_of_last_actual.isoformat().replace('-', '')}.tif")

        return dmc_key

    def get_previous_dc_key(self, date: datetime, model_run_hour: int):
        day_of_last_actual = self.determine_last_fwi_date(date, model_run_hour)
...
class FWIInput:
    ...
    def __post_init__(self):
        ...
        self.dmc_key = self.s3_key_fetcher.get_previous_dmc_key(self.start_time_utc, self.model_run_hour)
        self.dc_key = self.s3_key_fetcher.get_previous_dc_key(self.start_time_utc, self.model_run_hour)
    ...
    async def check_keys_exist(self):
        ...
        self.s3_keys_exist = await all_objects_exist(self.dmc_key, self.temp_key, self.rh_key, self.wind_spd_key, self.precip_key)
        return self.s3_keys_exist

    def get_gdal_s3_paths(self):
        ...
        return dmc_s3, temp_s3, rh_s3, precip_s3

    def open_gdal_datasets(self):
        """Open the GDAL datasets for all required weather data."""
        dmc_s3, temp_s3, rh_s3, precip_s3 = self.get_gdal_s3_paths()

        prev_fwi_ds = gdal.Open(dmc_s3)
        temp_ds = gdal.Open(temp_s3)
        rh_ds = gdal.Open(rh_s3)
        precip_ds = gdal.Open(precip_s3)

        return prev_fwi_ds, temp_ds, rh_ds, precip_ds

    def warp_datasets_to_match_fwi(self):
        """Process and warp the datasets to match the reference dataset."""
        with TemporaryDirectory() as tempdir:
            prev_fwi_ds, temp_ds, rh_ds, precip_ds = self.open_gdal_datasets()
            ...
            transformed_temp_ds = warp_to_match_raster(temp_ds, prev_fwi_ds, warped_temp_path, GDALResamplingMethod.BILINEAR)
            transformed_rh_ds = warp_to_match_raster(rh_ds, prev_fwi_ds, warped_rh_path, GDALResamplingMethod.BILINEAR)
            transformed_precip_ds = warp_to_match_raster(precip_ds, prev_fwi_ds, warped_precip_path, GDALResamplingMethod.BILINEAR)

            temp_ds = None
            rh_ds = None
            precip_ds = None

            return prev_fwi_ds, transformed_temp_ds, transformed_rh_ds, transformed_precip_ds
