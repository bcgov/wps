...
class StationMachineLearning:
    ...
    def __init__(self, session: Session, model: PredictionModel, target_coordinate: List[float], station_code: int, max_learn_date: datetime):
        """
    ...
    def _add_sample_to_collection(self, prediction: ModelRunPrediction, actual: HourlyActual, sample_collection: SampleCollection):
    ...
    def _collect_data(self, start_date: datetime):
        ...
        query = get_actuals_left_outer_join_with_predictions(self.session, self.model.id, self.station_code, start_date, self.max_learn_date)
        ...
        for actual, prediction in query:
            if prev_actual != actual and prediction is not None:
                if (
                    ...
                    noon_prediction = construct_interpolated_noon_prediction(prev_prediction, prediction, SCALAR_MODEL_VALUE_KEYS)
    ...
    def _learn_precip_model(self, start_date):
        ...
        actual_daily_precip = get_accumulated_precip_by_24h_interval(self.session, self.station_code, start_datetime, end_datetime)
        # Get the model predicted values
        predicted_daily_precip = get_predicted_daily_precip(self.session, self.model, self.station_code, start_datetime, end_datetime)
    ...
    def predict_temperature(self, model_temperature: float, timestamp: datetime):
        ...
        if self.regression_models[hour].temperature_wrapper.good_model:
            return self.unwrap_np_float(self.regression_models[hour].temperature_wrapper.model.predict([[model_temperature]])[0])
        return None
    ...
    def predict_wind_speed(self, model_wind_speed: float, timestamp: datetime):
        ...
        if self.regression_models[hour].wind_speed_wrapper.good_model and model_wind_speed is not None:
            predicted_wind_speed = self.regression_models[hour].wind_speed_wrapper.model.predict([[model_wind_speed]])[0]
