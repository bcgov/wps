    ...
    for area in fire_centre.planning_areas:
        ...
        for index, prep_day_dailies in enumerate(dailies_in_prep):
            ...
            validatedDailies: List[ValidatedStationDaily] = list(map(
                lambda daily: (validate_station_daily(daily), prep_day_dailies)))
            all_dailies_valid = all(map(lambda validated_daily: (validated_daily.valid), validatedDailies))
            daily_result: DailyResult = DailyResult(
                dateISO=prep_day_dailies[0].date,
