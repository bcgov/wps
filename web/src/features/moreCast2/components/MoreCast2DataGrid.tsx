...
import { MORECAST2_GRID_COLUMNS } from 'features/moreCast2/components/morecast2GridColumns'
...
const MoreCast2DataGrid = ({ rows, setRows, fromTo }: MoreCast2DataGridProps) => {
  ...
  useEffect(() => {
    if (!isNull(colPrediction)) {
      const newRows: MoreCast2ForecastRow[] = rows.map(r => ({
        ...r,
        [colPrediction.colField]: { value: 42, choice: colPrediction.modelType }
      }))
      setRows(newRows)
    }
