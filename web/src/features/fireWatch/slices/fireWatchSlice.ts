...
import { FireWatch, FireWatchOutput, getActiveFireWatches, postFireWatchInput } from '@/features/fireWatch/fireWatchApi'
...
const fireWatchSlice = createSlice({
  ...
  reducers: {
    ...
    getFireWatchSuccess(
      state: FireWatchState,
      action: PayloadAction<{ fireWatchList: FireWatchOutput[] }>
    ) {
      state.error = null
...
export const { getFireWatchStart, getFireWatchFailed, getFireWatchSuccess, submitFireWatchStart, submitFireWatchFailed, submitFireWatchSuccess} = fireWatchSlice.actions

export default fireWatchSlice.reducer

export const fetchActiveFireWatches = (): AppThunk =>
  async dispatch => {
    try {
...
export const submitNewFireWatch = (fireWatch: FireWatch): AppThunk =>
	async dispatch => {
		try{
