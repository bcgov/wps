...
function createRaster() {
  const raster = new olSource.Raster({
    ...
    lib: {
      ISIcalc: (ffmc: number, ws: number, fbpMod: boolean = false): number => {
        /*
        #############################################################################
        # Description:
        #   Computes the Initial Spread Index From the FWI System. Equations are from
      ...
      calcROSColour: (ros: number, opacity: number): number[] => {
        if (ros < 0) {
          if (ros === -2) {
            // it's water - or something that doesn't burn.
            return [0, 0, 0, 0]
      ...
      ftlNumberToFtlCode: (ftlNumber: number): string | undefined => {
        switch (ftlNumber) {
          case 1:
            return 'C1'
          case 2:
      ...
      BEcalc: (FUELTYPE: string, BUI: number): number => {
        // #############################################################################
        // # Description:
        // #   Computes the Buildup Effect on Fire Spread Rate.
        // #
