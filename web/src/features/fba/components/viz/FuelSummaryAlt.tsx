...
export interface FuelTypeAltInfoSummary {
  area: number
...
const FuelSummaryAlt = ({ fireZoneFuelStats, selectedFireZoneUnit }: FuelSummaryAltProps) => {
  const theme = useTheme()
  const [fuelTypeInfoRollup, setFuelTypeInfoRollup] = useState<FuelTypeAltInfoSummary[]>([])
  ...
  useEffect(() => {
    ...
    const rollUp: FuelTypeAltInfoSummary[] = []
    ...
    for (const key in groupedFuelDetails) {
      ...
      if (groupedFuelDetail.length) {
        ...
        const fuelInfo: FuelTypeAltInfoSummary = {
          area,
