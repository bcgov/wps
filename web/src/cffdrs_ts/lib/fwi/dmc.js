 ...
 */
function dmc(dmc_yda, temp, rh, prec, lat, mon, lat_adjust) {
    if (lat_adjust === void 0) { lat_adjust = true; }
    // FIX: works for now
    var exp = Math.exp;
    var log = Math.log;
    // Reference latitude for DMC day length adjustment
    // 46N: Canadian standard, latitude >= 30N   (Van Wagner 1987)
    var ell01 = [6.5, 7.5, 9, 12.8, 13.9, 13.9, 12.4, 10.9, 9.4, 8, 7, 6];
    // 20N: For 30 > latitude >= 10
    var ell02 = [7.9, 8.4, 8.9, 9.5, 9.9, 10.2, 10.1, 9.7, 9.1, 8.6, 8.1, 7.8];
    // 20S: For -10 > latitude >= -30
    var ell03 = [10.1, 9.6, 9.1, 8.5, 8.1, 7.8, 7.9, 8.3, 8.9, 9.4, 9.9, 10.2];
    // 40S: For -30 > latitude
    var ell04 = [11.5, 10.5, 9.2, 7.9, 6.8, 6.2, 6.5, 7.4, 8.7, 10, 11.2, 11.8];
    ...
    var rk = 1.894 * (temp + 1.1) * (100 - rh) * ell01[mon - 1] * 1e-4;
    ...
    if (prec > 1.5) {
        ...
        var wmi = 20 + 280 / exp(0.023 * dmc_yda);
        //Eqs. 13a, 13b, 13c
        var b = dmc_yda <= 33
            ? 100 / (0.5 + 0.3 * dmc_yda)
            : dmc_yda <= 65
        ...
        var wmr = wmi + (1000 * rw) / (48.77 + b * rw);
