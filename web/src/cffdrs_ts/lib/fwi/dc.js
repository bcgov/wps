 ...
 */
function dc(dc_yda, temp, rh, prec, lat, mon, lat_adjust) {
    ...
    var fl01 = [-1.6, -1.6, -1.6, 0.9, 3.8, 5.8, 6.4, 5, 2.4, 0.4, -1.6, -1.6];
    // 20S: South of 20 degrees S
    var fl02 = [6.4, 5, 2.4, 0.4, -1.6, -1.6, -1.6, -1.6, -1.6, 0.9, 3.8, 5.8];
    // Near the equator, we just use 1.4 for all months.
    // Constrain temperature
    temp = temp < -2.8 ? -2.8 : temp;
    // Eq. 22 - Potential Evapotranspiration
    var pe = (0.36 * (temp + 2.8) + fl01[mon - 1]) / 2;
    ...
    var rw = 0.83 * ra - 1.27;
    // Eq. 19
    var smi = 800 * exp((-1 * dc_yda) / 400);
    // Alteration to Eq. 21
    var dr0 = dc_yda - 400 * log(1 + (3.937 * rw) / smi);
    dr0 = dr0 < 0 ? 0 : dr0;
    // if precip is less than 2.8 then use yesterday's DC
    var dr = prec <= 2.8 ? dc_yda : dr0;
    // Alteration to Eq. 23
    var dc1 = dr + pe;
    dc1 = dc1 < 0 ? 0 : dc1;
    return dc1;
