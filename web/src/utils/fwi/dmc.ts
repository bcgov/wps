...
export function dmc(
  ...
  if (lat_adjust) {
    rk = lat <= 30 && lat > 10 ? 1.894 * (temp + 1.1) * (100 - rh) * ell02[mon - 1] * 1e-4 : rk
    rk = lat <= -10 && lat > -30 ? 1.894 * (temp + 1.1) * (100 - rh) * ell03[mon - 1] * 1e-4 : rk
    rk = lat <= -30 && lat >= -90 ? 1.894 * (temp + 1.1) * (100 - rh) * ell04[mon - 1] * 1e-4 : rk
    rk = lat <= 10 && lat > -10 ? 1.894 * (temp + 1.1) * (100 - rh) * 9 * 1e-4 : rk
  }
  ...
  if (prec > 1.5) {
    ...
    //Eqs. 13a, 13b, 13c
    const b =
      dmc_yda <= 33 ? 100 / (0.5 + 0.3 * dmc_yda) : dmc_yda <= 65 ? 14 - 1.3 * log(dmc_yda) : 6.2 * log(dmc_yda) - 17.2
