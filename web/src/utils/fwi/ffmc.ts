 ...
 */
export function ffmc(ffmc_yda: number, temp: number, rh: number, ws: number, prec: number): number {
  // FIX: works for now
  const exp = Math.exp
  ...
  const ed = 0.942 * rh ** 0.679 + 11 * exp((rh - 100) / 10) + 0.18 * (21.1 - temp) * (1 - 1 / exp(rh * 0.115))
  // Eq. 5 Equilibrium moisture content from wetting
  const ew = 0.618 * rh ** 0.753 + 10 * exp((rh - 100) / 10) + 0.18 * (21.1 - temp) * (1 - 1 / exp(rh * 0.115))
  // Eq. 6a (ko) Log drying rate at the normal
  //  temperature of 21.1 C
  let z =
    wmo < ed && wmo < ew ? 0.424 * (1 - ((100 - rh) / 100) ** 1.7) + 0.0694 * sqrt(ws) * (1 - (100 - rh) / 100) ** 8 : 0
  ...
  //  temperature of 21.1 C
  z = wmo > ed ? 0.424 * (1 - (rh / 100) ** 1.7) + 0.0694 * sqrt(ws) * (1 - (rh / 100) ** 8) : z
