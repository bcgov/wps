...
import type { LayerVisibility } from '@/components/map/layerVisibility'
import { ADVISORY_ORANGE_FILL, ADVISORY_RED_FILL, HFI_ADVISORY, HFI_WARNING } from '@/featureStylers'
    ...
    <Grid>
      ...
      >
        <FormControlLabel
          ...
          label={
            <Typography variant="h2" sx={{ fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer' }}>
              {label}
            </Typography>
          }
          sx={{ paddingLeft: 2 }}
  ...
  return (
    <LegendGrid sx={{ padding: '0 0.5rem' }} data-testid={`asa-go-map-legend`}>
      ...
      <LegendItem
        ...
        onChange={(_, checked) => onLayerVisibilityChange(ZONE_STATUS_LAYER_NAME, checked)}
        subItems={zoneStatusSubItems}
      />
      <LegendItem
        label="HFI Potential (kW/m)"
        checked={layerVisibility[HFI_LAYER_NAME]}
        onChange={(_, checked) => onLayerVisibilityChange(HFI_LAYER_NAME, checked)}
