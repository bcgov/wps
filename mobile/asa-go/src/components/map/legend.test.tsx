...
describe('Legend', () => {
  ...
  it('renders the legend with defaults checked', async () => {
    const { getByTestId } = render(
      <Legend layerVisibility={defaultLayerVisibility} onLayerVisibilityChange={onLayerVisibilityChange} />
    )
  ...
  it('calls onLayerVisibilityChange when checkboxes are toggled', async () => {
    ...
    await waitFor(() => expect(onLayerVisibilityChange).toHaveBeenCalledWith(ZONE_STATUS_LAYER_NAME, false))

    await userEvent.click(hfiCheckbox)
    await waitFor(() => expect(onLayerVisibilityChange).toHaveBeenCalledWith(HFI_LAYER_NAME, false))
  })
})
