...
vi.mock('@mui/x-data-grid-pro', () => ({
  DataGridPro: ({ rows, columns }: { rows: unknown[]; columns: unknown[] }) => (
    // biome-ignore lint/a11y/useSemanticElements: test mock mirrors DataGridPro's grid role so getByRole('grid') queries keep working
    <div data-testid="data-grid-pro" role="grid">
      ...
      {rows.map((row, index) => {
        ...
        return (
          <div key={(rowData.id as string) || index} data-testid={`grid-row-${index}`}>
...
vi.mock('@/components/profile/CriticalHours', () => ({
  default: ({ start, end }: { start?: number; end?: number }) => (
    <div data-testid="critical-hours">{start !== undefined && end !== undefined ? `${start}:00 - ${end}:00` : '-'}</div>
  )
}))
...
describe('FuelSummary', () => {
  ...
  it('should render the DataGridPro when fuel data is available', () => {
    render(<FuelSummary fireZoneFuelStats={mockFireZoneFuelStats} selectedFireZoneUnit={mockFireShape} />)
    ...
    expect(screen.queryByText('No fuel type information available.')).not.toBeInTheDocument()
  })

  it('should show no data message when fuel stats are empty', () => {
    render(<FuelSummary fireZoneFuelStats={{}} selectedFireZoneUnit={mockFireShape} />)

    expect(screen.getByText('No fuel type information available.')).toBeInTheDocument()
  })

  it('should show no data message when selectedFireZoneUnit is undefined', () => {
    render(<FuelSummary fireZoneFuelStats={mockFireZoneFuelStats} selectedFireZoneUnit={undefined} />)
  ...
  it('should show no data message when fireZoneFuelStats is undefined', () => {
    render(
      <FuelSummary
        fireZoneFuelStats={undefined as unknown as Record<number, FireZoneFuelStats[]>}
  ...
  it('should show no data message when fuel details for selected zone are not available', () => {
    ...
    render(<FuelSummary fireZoneFuelStats={emptyFireZoneFuelStats} selectedFireZoneUnit={mockFireShape} />)
  ...
  it('should process fuel data correctly with different fire_shape_ids', () => {
    ...
    render(<FuelSummary fireZoneFuelStats={differentShapeFireStats} selectedFireZoneUnit={differentFireShape} />)
