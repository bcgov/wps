...
vi.mock('@/components/profile/ElevationLabel', () => ({
  default: ({ label }: { label: string }) => (
    <div data-testid={`elevation-label-${label.toLowerCase().replace(' ', '-')}`}>{label}</div>
  )
}))

vi.mock('@/components/profile/ElevationFlag', () => ({
  default: ({ id, percent, testId }: { id: string; percent: number; testId?: string }) => (
...
describe('ElevationStatus', () => {
  ...
  it('should render all three elevation zones', () => {
    ...
    expect(screen.getByTestId('elevation-label-upper-slope')).toBeInTheDocument()
    expect(screen.getByTestId('elevation-label-mid-slope')).toBeInTheDocument()
    expect(screen.getByTestId('elevation-label-valley-bottom')).toBeInTheDocument()
  ...
  it('should handle zero TPI values', () => {
    ...
    expect(screen.getByTestId('upper-slope')).toHaveAttribute('data-percent', '0')
    expect(screen.getByTestId('mid-slope')).toHaveAttribute('data-percent', '0')
    expect(screen.getByTestId('valley-bottom')).toHaveAttribute('data-percent', '0')
  })
  ...
  it('should handle mixed zero and non-zero values', () => {
    ...
    // Mid slope: 50/100 = 50%
    expect(screen.getByTestId('mid-slope')).toHaveAttribute('data-percent', '50')
  ...
  it('should round percentages correctly', () => {
    ...
    expect(screen.getByTestId('valley-bottom')).toHaveAttribute('data-percent', '33')
    expect(screen.getByTestId('mid-slope')).toHaveAttribute('data-percent', '67')
    expect(screen.getByTestId('upper-slope')).toHaveAttribute('data-percent', '71')
  })
