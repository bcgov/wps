...
describe('ElevationFlag', () => {
  it('should render the flag component', () => {
    const { container } = render(<ElevationFlag percent={75} testId="upper-slope" />)
  ...
  it('should handle different percentage values', () => {
    ...
    percentages.forEach(percent => {
      const { container } = render(<ElevationFlag testId="test" percent={percent} />)
      // Since maskId is now random, we need to find the element differently
      const mockFlag = container.querySelector('[data-testid^="mock-flag-elevation-flag-"]')
      expect(mockFlag).toBeInTheDocument()
