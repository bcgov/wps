  ...
  const fireZoneTPIStats = useMemo(() => {
    if (selectedFireCentre && !isNil(fireCentreTPIStats)) {
      ...
      return tpiStatsArray
        ? tpiStatsArray.find(stats => stats.fire_zone_id === selectedFireZoneUnit?.fire_shape_id)
        : undefined
    }
  }, [fireCentreTPIStats, selectedFireCentre, selectedFireZoneUnit?.fire_shape_id])

  const fireZoneFuelStats = useMemo(() => {
    if (hfiFuelStats && selectedFireZoneUnit && hfiFuelStats[selectedFireZoneUnit.fire_shape_id]?.fuel_area_stats) {
      return {
        [selectedFireZoneUnit.fire_shape_id]: hfiFuelStats[selectedFireZoneUnit.fire_shape_id].fuel_area_stats
      }
    }
    return {}
