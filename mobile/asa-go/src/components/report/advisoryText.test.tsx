...
const EXPECTED_RUN_DATETIME = DateTime.fromISO(TEST_RUN_DATETIME).toLocaleString(DateTime.DATETIME_FULL)
...
describe('AdvisoryText', () => {
  ...
  const assertInitialState = () => {
    ...
    expect(screen.queryByTestId('advisory-message-wind-speed')).toBeInTheDocument()
  ...
  it('should include fuel stats when their fuel area is above the 100 * 2000m * 2000m threshold', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByTestId('advisory-message-warning')).toHaveTextContent(
        mockFireZoneHFIStatsDictionary[20].fuel_area_stats[0].fuel_type.fuel_type_code
      )
    )
  })
  ...
  it('should render forDate as mmm/dd when different than issue date', () => {
    ...
    expect(bulletinIssueDate).toHaveTextContent(`Issued on ${EXPECTED_RUN_DATETIME} for ${EXPECTED_FOR_DATE}.`)
  })
  ...
  it("should render forDate as 'today' when forDate parameter matches today's date", () => {
    ...
    const bulletinIssueDate = queryByTestId('bulletin-issue-date')
    expect(bulletinIssueDate).toBeInTheDocument()
    expect(bulletinIssueDate).toHaveTextContent(`Issued on ${EXPECTED_RUN_DATETIME} for today.`)
  })
  ...
  it('should render slash warning when critical hours duration is greater than 12 hours', async () => {
    ;(useRunParameterForDate as Mock).mockReturnValue(testRunParameter)
    const filteredCriticalHoursStats = cloneDeep(mockFireZoneHFIStatsDictionary)
    filteredCriticalHoursStats[20].fuel_area_stats[0].critical_hours.end_time = 22
    ;(useFilteredHFIStatsForDate as Mock).mockReturnValue(filteredCriticalHoursStats)
    const store = getInitialStore()
    render(
      <Provider store={store}>
        <AdvisoryText
