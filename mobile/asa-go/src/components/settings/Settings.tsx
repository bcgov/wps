...
const Settings = ({ activeTab }: SettingsProps) => {
  ...
  const orderedFireCentres = useMemo<FireCentreInfo[]>(() => {
    ...
    const sorted = fireCentreInfos.toSorted((a, b) => a.fire_centre_name.localeCompare(b.fire_centre_name))

    if (!isNil(pinnedFireCentre)) {
      // Move pinned item to the top
      const index = sorted.findIndex(fc => fc.fire_centre_name === pinnedFireCentre)
    ...
    const finalSorted: FireCentreInfo[] = sorted.map(fc => {
      return {
        ...
        fire_zone_units: fc.fire_zone_units.toSorted((a, b) => a.name.localeCompare(b.name))
      }
    })
  ...
  const renderOfflineMessage = () => {
    ...
    return (
      <Alert severity="warning" sx={{ mx: 1, my: 1 }} data-testid="notifications-permission-warning">
  ...
  const renderDeviceIdErrorBanner = () => {
    ...
    return (
      <Alert severity="warning" sx={{ mx: 1, my: 1 }} data-testid="device-id-error-banner">
    ...
    if (shouldShow) {
      return (
        <Alert severity="warning" sx={{ mx: 1, my: 1 }} data-testid="notifications-permission-warning">
          ...
          Notifications are currently disabled in your system settings. To receive alerts, enable notifications for this
