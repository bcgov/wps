...
const getAccordionButton = () => screen.getByText(FIRE_CENTRE_LABEL).closest('button')

const getAccordion = (fireCentreName: string) => screen.getByLabelText(`accordion-${fireCentreName}`)

const getAllCheckbox = (fireCentreName: string) => within(getAccordion(fireCentreName)).getByLabelText('All')
...
const KAMLOOPS_FIRE_ZONE_IDS = mockFireCentreInfo.fire_zone_units.map(fireZoneUnit => fireZoneUnit.id)
...
describe('SubscriptionAccordion', () => {
  ...
  it('renders correctly with fire centre name', () => {
    ...
    render(
      <Provider store={store}>
        <SubscriptionAccordion defaultExpanded={false} disabled={false} fireCentreInfo={mockFireCentreInfo} />
      </Provider>
    )
  ...
  it('renders as expanded when defaultExpanded is true', () => {
    ...
    render(
      <Provider store={store}>
        <SubscriptionAccordion disabled={false} fireCentreInfo={mockFireCentreInfo} defaultExpanded={true} />
      </Provider>
    )
  ...
  it('renders as collapsed when defaultExpanded is false', () => {
    ...
    render(
      <Provider store={store}>
        <SubscriptionAccordion disabled={false} fireCentreInfo={mockFireCentreInfo} defaultExpanded={false} />
      </Provider>
    )
  ...
  it('is disabled when disabled prop is true', () => {
    ...
    render(
      <Provider store={store}>
        <SubscriptionAccordion disabled={true} fireCentreInfo={mockFireCentreInfo} defaultExpanded={false} />
      </Provider>
    )
    ...
    const toggleSwitches = within(accordion as HTMLElement).getAllByTestId('loading-switch')
  ...
  it('shows a snackbar error when a subscription toggle fails', async () => {
    ...
    render(
      <Provider store={store}>
        <SubscriptionAccordion defaultExpanded={true} disabled={false} fireCentreInfo={mockFireCentreInfo} />
      </Provider>
    )
    ...
    await act(async () => {
      fireEvent.click(screen.getByLabelText(`Toggle subscription for ${firstZone.name}`))
    })
  ...
  it('pins the fire centre when pin button is clicked and not already pinned', async () => {
    ...
    await waitFor(() => {
      expect(store.getState().settings.pinnedFireCentre).toEqual(mockFireCentreInfo.fire_centre_name)
    })
  })
  ...
  it("selects all fire zone units when 'All' checkbox is checked", async () => {
    ...
    await waitFor(() => {
      expect(store.getState().settings.subscriptions).toEqual(KAMLOOPS_FIRE_ZONE_IDS)
    })
  })
  ...
  it('shows indeterminate state when some fire zone units are selected', () => {
    ...
    expect(store.getState().settings.subscriptions).toContain(mockFireCentreInfo.fire_zone_units[0].id)
    expect(store.getState().settings.subscriptions).not.toContain(mockFireCentreInfo.fire_zone_units[1].id)
  })
  ...
  it('All checkbox on accordion works independently', async () => {
    ...
    render(
      <Provider store={store}>
        <SubscriptionAccordion disabled={false} fireCentreInfo={mockFireCentreInfos[0]} defaultExpanded={true} />
        <SubscriptionAccordion disabled={false} fireCentreInfo={mockFireCentreInfos[1]} defaultExpanded={true} />
      </Provider>
    )

    // Confirm that PG fire zones switches are checked as per initial redux state
    const switchPG1 = await screen.findByLabelText('Toggle subscription for G1-Prince George Zone')
    const switchPG2 = await screen.findByLabelText('Toggle subscription for G3-Robson Valley Zone')
    await waitFor(() => {
    ...
    const switchK1 = await screen.findByLabelText('Toggle subscription for K2-Kamloops Zone (Kamloops)')
    const switchK2 = await screen.findByLabelText('Toggle subscription for K5-Penticton Zone')
