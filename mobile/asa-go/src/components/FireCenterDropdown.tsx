...
const FireCenterDropdown = ({
  ...
  const getDisplayName = (name: string): string => name.replace('Fire Centre', '').trim()
  ...
  return (
    <FormControl variant="outlined" size="small" sx={{ minWidth: 175, height: BUTTON_HEIGHT }}>
      ...
      >
        {fireCentreOptions.map(option => {
          ...
          return (
            <MenuItem key={option.name} value={option.name} sx={{ fontWeight: 'bold', color: MAP_BUTTON_GREY }}>
              {displayName}
            </MenuItem>
          )
        })}
