...
vi.mock('@/components/SideNavigationListItem', () => ({
  default: ({
    ...
    <div data-testid={`nav-item-${navItem.toLowerCase()}`} data-navitem={navItem} data-tab={tab}>
      {navItem}
    </div>
  )
}))
...
describe('SideNavigation', () => {
  ...
  it('passes selected tab information to SideNavigationListItem', () => {
    ...
    expect(screen.getByTestId('nav-item-map')).toHaveAttribute('data-tab', NavPanel.ADVISORY)
    expect(screen.getByTestId('nav-item-advisory')).toHaveAttribute('data-tab', NavPanel.ADVISORY)
    expect(screen.getByTestId('nav-item-profile')).toHaveAttribute('data-tab', NavPanel.ADVISORY)
  })
  ...
  it('renders correctly with different selected tabs', () => {
    ...
    navItems.forEach(selectedTab => {
      const { unmount } = render(<SideNavigation tab={selectedTab} setTab={mockSetTab} />)
