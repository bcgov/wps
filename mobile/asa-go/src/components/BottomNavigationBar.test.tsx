...
describe('BottomNavigationBar', () => {
  ...
  it('renders all navigation actions', () => {
    ...
    expect(screen.getByRole('button', { name: NavPanel.PROFILE })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: NavPanel.ADVISORY })).toBeInTheDocument()
  })

  it('applies selected class to the active tab', () => {
    const { rerender } = render(<BottomNavigationBar tab={NavPanel.MAP} setTab={vi.fn()} />)

    expect(screen.getByRole('button', { name: NavPanel.MAP })).toHaveClass('Mui-selected')
    expect(screen.getByRole('button', { name: NavPanel.PROFILE })).not.toHaveClass('Mui-selected')

    rerender(<BottomNavigationBar tab={NavPanel.PROFILE} setTab={vi.fn()} />)

    expect(screen.getByRole('button', { name: NavPanel.PROFILE })).toHaveClass('Mui-selected')
    expect(screen.getByRole('button', { name: NavPanel.MAP })).not.toHaveClass('Mui-selected')
  })
