...
const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    getFireCentreInfoSuccess(state: SettingsState, action: PayloadAction<FireCentreInfo[]>) {
      state.error = null
      state.fireCentreInfos = action.payload
      state.loading = false
    },
    setPinnedFireCentre(state: SettingsState, action: PayloadAction<string | null>) {
...
export const fetchFireCentreInfo = (): AppThunk => async (dispatch, getState) => {
  ...
  if (!isNull(cachedFireCentreInfo)) {
    ...
    if (
      lastUpdated.plus({ hours: FIRE_CENTRE_INFO_CACHE_EXPIRATION }) > today ||
      !networkStatus.networkStatus.connected
    ) {
      dispatch(getFireCentreInfoSuccess(cachedFireCentreInfo.data as FireCentreInfo[]))
      return
    }
  }
