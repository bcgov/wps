...
const fireCentresSlice = createSlice({
  ...
  reducers: {
    ...
    getFireCentresSuccess(state: FireCentresState, action: PayloadAction<FireCentre[]>) {
      state.error = null
...
export const fetchFireCentres = (): AppThunk => async (dispatch, getState) => {
  ...
  if (!isNull(cachedFireCentres)) {
    ...
    if (lastUpdated.plus({ hours: FIRE_CENTRES_CACHE_EXPIRATION }) > today || !networkStatus.networkStatus.connected) {
