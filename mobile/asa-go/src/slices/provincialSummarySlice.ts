...
const provincialSummarySlice = createSlice({
  ...
  reducers: {
    ...
    getProvincialSummaryFailed(
      state: ProvincialSummaryState,
      action: PayloadAction<string>
    ) {
      state.error = action.payload;
      state.loading = false;
    },
    getProvincialSummarySuccess(
      state: ProvincialSummaryState,
      action: PayloadAction<ProvincialSummaryResponse>
    ) {
      state.error = null;
  ...
  async (dispatch) => {
    if (!isUndefined(run_datetime) && !isNull(run_datetime)) {
      try {
        ...
        const fireShapeAreas = await getProvincialSummary(
          runType,
          run_datetime,
          for_date
        );
...
const selectFireShapeAreaDetails = (state: RootState) =>
  state.provincialSummary;
