...
const fireShapeAreasSlice = createSlice({
  ...
  reducers: {
    ...
    getFireShapeAreasFailed(
      state: FireZoneAreasState,
      action: PayloadAction<string>
    ) {
      state.error = action.payload;
      state.loading = false;
    },
    getFireShapeAreasSuccess(
      state: FireZoneAreasState,
      action: PayloadAction<FireShapeAreaListResponse>
    ) {
      state.error = null;
...
export const {
  getFireShapeAreasStart,
  getFireShapeAreasFailed,
  getFireShapeAreasSuccess,
} = fireShapeAreasSlice.actions;
  ...
  async (dispatch) => {
    if (!isUndefined(run_datetime) && !isNull(run_datetime)) {
      try {
        ...
        const fireShapeAreas = await getFireShapeAreas(
          runType,
          run_datetime,
          for_date
        );
