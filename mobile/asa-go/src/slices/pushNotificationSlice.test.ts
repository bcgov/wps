...
describe('pushNotificationSlice', () => {
  const makeState = (overrides: Partial<PushNotificationState> = {}): PushNotificationState => ({
    ...initialState,
    ...overrides
  })
  ...
  describe('reducers', () => {
    it('returns the initial state', () => {
      expect(pushNotificationReducer(undefined, { type: 'unknown' })).toEqual(initialState)
    })
    ...
    it('handles setPushNotificationPermission', () => {
      const next = pushNotificationReducer(makeState(), setPushNotificationPermission('granted'))
      expect(next.pushNotificationPermission).toBe('granted')
    })

    it('handles setRegisteredFcmToken to a value', () => {
      const next = pushNotificationReducer(makeState(), setRegisteredFcmToken('my-token'))
      expect(next.registeredFcmToken).toBe('my-token')
    })

    it('handles setRegisteredFcmToken to null', () => {
      const next = pushNotificationReducer(makeState({ registeredFcmToken: 'my-token' }), setRegisteredFcmToken(null))
  ...
  describe('thunks', () => {
    ...
    describe('registerDevice', () => {
      ...
      it('does not set token when retryWithBackoff exhausts retries', async () => {
        ...
        ;(Device.getId as Mock).mockResolvedValue({ identifier: 'device-id' })
        ;(Capacitor.getPlatform as Mock).mockReturnValue('ios')
        ;(registerToken as Mock).mockRejectedValue(new Error('persistent error'))
