...
import { type HFIPMTilesFileVectorOptions, PMTilesFileVectorSource } from '@/utils/pmtilesVectorSource'
...
export const createLocalBasemapVectorLayer = async () => {
  const localBasemapSource = await PMTilesFileVectorSource.createBasemapSource(new PMTilesCache(Filesystem), {
...
export const createHFILayer = async (
  ...
  const hfiVectorSource = await PMTilesFileVectorSource.createHFILayer(new PMTilesCache(Filesystem), {
