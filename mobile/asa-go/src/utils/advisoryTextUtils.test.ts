...
import type { FireZoneHFIStats } from '@/api/fbaAPI'
...
const fireZoneFuelStats: FireZoneHFIStats = {
  fuel_area_stats: [fuelAreaStatsD, fuelAreaStatsB, fuelAreaStatsA, fuelAreaStatsC],
  min_wind_stats: []
}
...
describe('getTopFuelsByArea', () => {
  it('should return the top fuels by area before start of core season, correctly handling both advisory and warning hfi pixels', () => {
    ...
    expect(result.includes(fuelAreaStatsA))
    expect(result.includes(fuelAreaStatsB))
    expect(result.includes(fuelAreaStatsC))
  })
