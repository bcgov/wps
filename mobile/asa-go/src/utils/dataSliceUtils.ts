...
export const fetchHFIStatsForRunParameter = async (runParameter: RunParameter): Promise<FireZoneHFIStatsDictionary> => {
...
export const fetchHFIStats = async (
  ...
  const hfiStatsForToday = await fetchHFIStatsForRunParameter(runParameters[todayKey])
  const hfiStatsForTommorow = await fetchHFIStatsForRunParameter(runParameters[tomorrowKey])
  const hfiStats = shapeDataForCaching(todayKey, tomorrowKey, runParameters, hfiStatsForToday, hfiStatsForTommorow)
  return hfiStats as CacheableData<FireZoneHFIStatsDictionary>
}

export const fetchTpiStatsForRunParameter = async (runParameter: RunParameter): Promise<FireZoneTPIStats[]> => {
  if (isNil(runParameter)) {
    return []
  }
...
export const fetchTpiStats = async (
  ...
  const tpiStatsForToday = await fetchTpiStatsForRunParameter(runParameters[todayKey])
  const tpiStatsForTommorow = await fetchTpiStatsForRunParameter(runParameters[tomorrowKey])
  const tpiStats = shapeDataForCaching(todayKey, tomorrowKey, runParameters, tpiStatsForToday, tpiStatsForTommorow)
  return tpiStats as CacheableData<FireZoneTPIStats[]>
}

export const fetchProvincialSummary = async (runParameter: RunParameter): Promise<FireShapeStatusDetail[]> => {
  if (isNil(runParameter)) {
    return []
  }
...
export const fetchProvincialSummaries = async (
  ...
  const todayProvincialSummary = await fetchProvincialSummary(runParameters[todayKey])
  const tomorrowProvincialSummary = await fetchProvincialSummary(runParameters[tomorrowKey])
