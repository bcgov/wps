...
const FIRE_ZONE_UNITS_WITH_REDUNDANT_LOCATION = new Set(['K2-Kamloops|(Kamloops)', 'K4-Vernon|(Vernon)'])
...
export const nameFormatter = (input: string, suffix: string, toUpper: boolean) => {
...
export const fireZoneUnitNameFormatter = (input: string | undefined) => {
  ...
  const isRedundantLocation = FIRE_ZONE_UNITS_WITH_REDUNDANT_LOCATION.has(`${normalizedBaseName}|${bracketedSuffix}`)

  const output =
    bracketedSuffix && !isRedundantLocation ? `${formattedBaseName}\n${bracketedSuffix}` : formattedBaseName
