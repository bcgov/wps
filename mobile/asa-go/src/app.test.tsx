...
describe('App', () => {
  ...
  describe('InfoBar rendering', () => {
    ...
    it('shows INFO status when network is connected', async () => {
      ...
      await waitFor(() => expect(screen.getByTestId('info-bar')).toHaveAttribute('data-status', 'info'))
    })
  })
  ...
  describe('pendingNotificationData effect', () => {
    ...
    it('does not navigate when advisory_date does not match today', async () => {
      ...
      expect(store.getState().pushNotification.pendingNotificationData).toBeNull()
    })
    ...
    it('does not navigate when fire centre is not found', async () => {
      ...
      expect(screen.getByTestId('asa-go-map')).toBeInTheDocument()
      expect(screen.queryByTestId('advisory')).not.toBeInTheDocument()
      expect(store.getState().pushNotification.pendingNotificationData).not.toBeNull()
    })
