import { type Dictionary, groupBy, isNil } from 'lodash'
import type { DateTime } from 'luxon'
import { useMemo } from 'react'
import { useSelector } from 'react-redux'
import type { FireShapeStatusDetail, FireZoneHFIStatsDictionary, FireZoneTPIStats } from '@/api/fbaAPI'
import { selectHFIStats, selectProvincialSummaries, selectTPIStats } from '@/store'
 ...
 */
export const useFilteredHFIStatsForDate = (forDate: DateTime): FireZoneHFIStatsDictionary => {
  const hfiStats = useSelector(selectHFIStats)
  return useMemo(() => {
    const forDateString = forDate?.toISODate()
    if (isNil(forDate) || isNil(forDateString) || isNil(hfiStats?.[forDateString]?.data)) {
      return []
    }
 ...
 */
export const useProvincialSummaryForDate = (forDate: DateTime): Dictionary<FireShapeStatusDetail[]> | undefined => {
 ...
 */
export const useProvincialSummaryZonesForDate = (forDate: DateTime): FireShapeStatusDetail[] | undefined => {
  const provincialSummaries = useSelector(selectProvincialSummaries)
  return useMemo(() => {
    const forDateString = forDate?.toISODate()
    if (isNil(forDate) || isNil(forDateString) || isNil(provincialSummaries?.[forDateString]?.data)) {
...
export const useTPIStatsForDate = (forDate: DateTime): FireZoneTPIStats[] => {
  ...
  return useMemo(() => {
    const forDateString = forDate?.toISODate()
    if (isNil(forDate) || isNil(forDateString) || isNil(tpiStats?.[forDateString]?.data)) {
      return []
    }
