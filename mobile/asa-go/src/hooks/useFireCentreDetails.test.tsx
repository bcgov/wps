...
import { type RunParameter, RunType } from 'api/fbaAPI'
...
describe('useFireCentreDetails', () => {
  it('returns grouped and sorted fire shape details for a selected fire center', () => {
    ...
    const { result } = renderHook(() => useFireCentreDetails(mockFireCentre, testDate), {
  ...
  it('returns an empty array if no fire centre is selected', () => {
    ...
    const { result } = renderHook(() => useFireCentreDetails(undefined, testDate), {
      wrapper: ({ children }) => <Provider store={store}>{children}</Provider>
    })
