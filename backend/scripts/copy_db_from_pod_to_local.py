...
def list_projects() -> None:
    ...
    result = subprocess.run(
        ["oc", "get", "projects"], stdout=subprocess.PIPE, check=True, text=True
    )
    print(result.stdout)
...
def list_cluster_members(project: str, pod: str) -> None:
    ...
    result = subprocess.run(
        [*oc_rsh(project, pod), "patronictl", "list"], stdout=subprocess.PIPE, check=True, text=True
    )
...
def create_lock_file(project: str, pod: str, database: str) -> None:
    ...
    try:
        ...
        subprocess.run(
            [*oc_rsh(project, pod), "mkdir", lock_file],
            stdout=subprocess.PIPE,
            check=True,
            text=True,
        )
        ...
        print(
            "failed to create lock {}:{} - another copy may be in progress".format(pod, lock_file)
        )
        raise
...
def dump_database(project: str, pod: str, database: str, mode: Mode, num_days: int) -> List[str]:
    ...
    if mode == Mode.partial:
        ...
        for table in tables:
            ...
            sql_command = (
                'psql {database} -c "\\copy (SELECT * FROM {table} WHERE '
                "prediction_timestamp > current_date -5) to '{csv_file}' with csv\"\n"
            ).format(database=database, table=table, csv_file=csv_file)
        ...
        for table_name, values in tables_dict.items():
            ...
            sql_command = (
                ...
                table=table_name,
                csv_file=csv_file,
                max_days_to_learn=values["date_offset"],
...
def copy_files_to_local(project: str, pod: str, files: List[str]) -> None:
    ...
    for filename in files:
        ...
        result = subprocess.run(
            [
                *oc_n_project(project),
                "cp",
                "{}:{}.gz".format(pod, filename),
                "./{}.gz".format(filename),
            ],
            stdout=subprocess.PIPE,
            check=True,
