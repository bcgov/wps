...
from scripts.percentile_calculator.s3_to_dataframe import (
    load_all_csv_to_dataframe,
    get_csv_list_from_s3,
    filter_wx_and_station_csv,
)


RECENT_YEAR = 2023  # the most recent year that has the core fire season recorded.
RANGES = [
    (RECENT_YEAR - 29, RECENT_YEAR),
    (RECENT_YEAR - 19, RECENT_YEAR),
...
async def main():
    ...
    for start_year, end_year in RANGES:
        ...
        for station in stations:
            ...
            if station_df.empty:
                print("Data not available for {} creating empty summary...".format(station_code))
...
def create_null_summary(station, year_range: list):
    ...
    return {"ffmc": None, "isi": None, "bui": None, "years": year_range, "station": station}
...
def remove_data_outside_fire_season(df: pd.DataFrame, season: dict):
    ...
    outside_month = (df["month"] < season["start_month"]) | (df["month"] > season["end_month"])
    df = df.loc[~outside_month]

    before_start = (df["month"] == season["start_month"]) & (df["day"] < season["start_day"])
    df = df.loc[~before_start]

    after_end = (df["month"] == season["end_month"]) & (df["day"] > season["end_day"])
    df = df.loc[~after_end]
...
def calculate_percentile(df: pd.DataFrame, percentile: float) -> dict:
    ...
    ffmc = df[df["FINE_FUEL_MOISTURE_CODE"].notnull()]["FINE_FUEL_MOISTURE_CODE"].quantile(
        percentile
    )
...
def get_years_for_valid_fwi_values(df: pd.DataFrame) -> list:
    ...
    data_years = list(set(ffmc_data_years) | set(bui_data_years) | set(isi_data_years))
    data_years.sort()
