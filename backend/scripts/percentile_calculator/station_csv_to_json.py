...
def process_stations(
    station_df: pd.DataFrame, ecodivisions: gpd.GeoDataFrame, core_seasons: dict
) -> list:
    ...
    for _, row in station_df.iterrows():
        ...
        if row["STATION_CODE"] == "447":
            ...
            ecodivision_name = fetch_ecodivision_name(
                row["LATITUDE"], row["LONGITUDE"], ecodivisions
            )

        if ecodivision_name is not None:
            core_season = core_seasons.get(ecodivision_name, core_seasons["DEFAULT"]).get(
                "core_season"
            )
        else:
