...
from app.hfi.pdf_template import CSS_PATH, PDFTemplateName
...
def generate_html(
    ...
    rendered_output += generate_daily(
        result,
        idir,
        datetime_generated,
        planning_area_dict,
...
def generate_pdf(
    ...
    rendered_output, fire_centre_name = generate_html(
        result, fire_centres, idir, datetime_generated, jinja_env, fuel_types
    )
...
def generate_prep(
    ...
    prep_pdf_data, dates, date_range = response_2_prep_cycle_jinja_format(
        result, planning_area_dict, station_dict, fuel_types
    )
