...
from cffdrs.cfb_calc import critical_surface_intensity, crown_fraction_burned as _crown_fraction_burned, surface_fire_rate_of_spread
...
def foliar_moisture_content(lat: int, long: int, elv: float, day_of_year: int,
    ...
    """Computes FMC."""
    logger.debug('calling FMCcalc(LAT=%s, LONG=%s, ELV=%s, DJ=%s, D0=%s)', lat,
                 long, elv, day_of_year, date_of_minimum_foliar_moisture_content)
    # FMCcalc expects longitude to always be a positive number.
    if long < 0:
        long = -long
    return _foliar_moisture_content(lat=lat, long=long, elv=elv,
...
def fine_fuel_moisture_code(ffmc: float, temperature: float, relative_humidity: float,
    ...
    if temperature is None or relative_humidity is None or precipitation is None:
        return None
    return _fine_fuel_moisture_code(ffmc_yda=ffmc, temp=temperature, rh=relative_humidity,
                                    ws=wind_speed, prec=precipitation)
...
def duff_moisture_code(dmc: float, temperature: float, relative_humidity: float,
    ...
    if month is None:
        month = 7
    return _duff_moisture_code(dmc_yda=dmc, temp=temperature, rh=relative_humidity,
                               prec=precipitation, lat=latitude, mon=month,
                               lat_adjust=latitude_adjust)


def drought_code(dc: float, temperature: float, relative_humidity: float, precipitation: float,
                 latitude: float = 55, month: int = 7, latitude_adjust: bool = True):
