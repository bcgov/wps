...
async def test_m2_75conifer_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 8266.708, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 7.845, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.663, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 5.249, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 12.751, abs_tol=0.1
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 2.016, abs_tol=0.001
    )
