...
async def test_m3_100deadfir_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 42023.894, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 31.058, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.999, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 11.836, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 243.795, abs_tol=0.1
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 42.697, abs_tol=0.001
    )
