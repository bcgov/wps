...
async def test_m3_60deadfir_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 24277.177, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 19.355, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.979, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 8.996, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 94.034, abs_tol=0.1
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 16.431, abs_tol=0.001
    )
