...
async def test_m1_50conifer_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 4876.527, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 6.069, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.398, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 4.032, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 4.433, abs_tol=0.1
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 0.521, abs_tol=0.001
    )
