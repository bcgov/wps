...
async def test_c1_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 528.189, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 1.246, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.0, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 1.327, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 0.241, abs_tol=0.001
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 0.043, abs_tol=0.001
    )
...
@pytest.mark.anyio
@pytest.mark.usefixtures("mock_jwt_decode")
async def test_c1_no_daily_data_request_response(
    anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.setattr(ClientSession, "get", default_mock_client_get)
    ...
    response = await async_client.post(
        ...
        json={
            "date": "2021-07-05",
            "stations": [{"id": 0, "station_code": 836, "fuel_type": "C1", "crown_base_height": 2}],
        },
    )
...
@pytest.mark.anyio
@pytest.mark.usefixtures("mock_jwt_decode")
async def test_c1_forecast_request_response(
    anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.setattr(ClientSession, "get", default_mock_client_get)
    ...
    response = await async_client.post(
        ...
        json={
            "date": "2021-07-06",
            "stations": [{"id": 0, "station_code": 230, "fuel_type": "C1", "crown_base_height": 2}],
        },
    )
    ...
    assert math.isclose(
        response.json()["stations"][0]["fine_fuel_moisture_code"], 93.258, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["drought_code"], 340.544, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["initial_spread_index"], 10.810, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["build_up_index"], 117.899, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["duff_moisture_code"], 103.923, abs_tol=0.001
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 2324.329, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 4.249, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.493, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 2.784, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 2.854, abs_tol=0.001
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 0.509, abs_tol=0.001
    )
...
@pytest.mark.anyio
@pytest.mark.usefixtures("mock_jwt_decode")
async def test_c1_request_multiple_response(
    anyio_backend, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.setattr(ClientSession, "get", default_mock_client_get)
    ...
    response = await async_client.post(
        ...
        json={
            "date": "2021-07-05",
            "stations": [
                {"id": 0, "station_code": 230, "fuel_type": "C1", "crown_base_height": 2},
                {"id": 1, "station_code": 230, "fuel_type": "C1", "crown_base_height": 2},
                {"id": 2, "station_code": 230, "fuel_type": "C1", "crown_base_height": 3},
            ],
        },
    )
    ...
    for idx, station in enumerate(response.json()["stations"]):
        ...
        assert math.isclose(station["fine_fuel_moisture_code"], 90.638, abs_tol=0.001)
        assert math.isclose(station["drought_code"], 340.544, abs_tol=0.001)
        assert math.isclose(station["initial_spread_index"], 7.462, abs_tol=0.001)
        ...
        assert math.isclose(station["fire_weather_index"], 27.792, abs_tol=0.001)
        assert math.isclose(station["head_fire_intensity"], 528.189, abs_tol=0.001)
        assert math.isclose(station["rate_of_spread"], 1.246, abs_tol=0.001)
        assert math.isclose(station["percentage_crown_fraction_burned"], 0.0, abs_tol=0.001)
        assert math.isclose(station["flame_length"], 1.327, abs_tol=0.001)
        assert math.isclose(station["sixty_minute_fire_size"], 0.241, abs_tol=0.001)
        assert math.isclose(station["thirty_minute_fire_size"], 0.043, abs_tol=0.001)
