...
async def test_m4_100deadfir_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 20799.353, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 15.510, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.948, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 8.327, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 57.208, abs_tol=0.001
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 9.957, abs_tol=0.001
    )
