...
async def test_m4_60deadfir_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 11599.508, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 9.450, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.792, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 6.218, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 20.412, abs_tol=0.1
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 3.430, abs_tol=0.001
    )
