...
async def test_s1_forecast_request_response(
    ...
    response = await async_client.post(
        ...
        json={
            "date": "2021-07-06",
            "stations": [{"id": 0, "station_code": 230, "fuel_type": "S1"}],
        },
    )
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 41822.354, abs_tol=0.01
    )
    ...
    assert math.isclose(response.json()["stations"][0]["flame_length"], 11.807, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 85.899, abs_tol=0.001
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 15.330, abs_tol=0.001
    )
