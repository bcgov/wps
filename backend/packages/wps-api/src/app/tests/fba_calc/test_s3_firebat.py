...
async def test_c1_request_response(
    ...
    response = await async_client.post(
        ...
        json={
            "date": "2021-07-05",
            "stations": [{"id": 0, "station_code": 230, "fuel_type": "S3"}],
        },
    )
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 55905.091, rel_tol=1e-5, abs_tol=0.1
    )
    ...
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 7.398, abs_tol=0.001
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 1.320, abs_tol=0.001
    )
