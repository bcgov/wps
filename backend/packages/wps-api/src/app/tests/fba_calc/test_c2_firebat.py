...
async def test_c2_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["head_fire_intensity"], 13692.788, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["rate_of_spread"], 10.328, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.885, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 6.756, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 24.469, abs_tol=0.001
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 4.213, abs_tol=0.001
    )
