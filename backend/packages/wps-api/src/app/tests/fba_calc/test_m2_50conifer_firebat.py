...
async def test_m2_50conifer_request_response(
    ...
    assert math.isclose(
        response.json()["stations"][0]["percentage_crown_fraction_burned"], 0.290, abs_tol=0.001
    )
    assert math.isclose(response.json()["stations"][0]["flame_length"], 3.755, abs_tol=0.001)
    assert math.isclose(
        response.json()["stations"][0]["sixty_minute_fire_size"], 2.957, abs_tol=0.1
    )
    assert math.isclose(
        response.json()["stations"][0]["thirty_minute_fire_size"], 0.325, abs_tol=0.001
    )
