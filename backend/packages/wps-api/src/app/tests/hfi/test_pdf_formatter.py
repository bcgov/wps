...
def test_get_sorted_dates_same():
    ...
    date_range: DateRange = DateRange(start_date=start_date, end_date=end_date)
...
def test_get_merged_station_data():
    weather_station_1 = WeatherStation(
        code=1,
        station_props=WeatherStationProperties(name="s1", wfwx_station_uuid="1", elevation=1),
    )
...
def test_response_2_prep_cycle_jinja_format():
    with open(test_hfi_result, "r") as hfi_result, open(test_fcs, "r") as fcs:
        ...
        fuel_types = {
            id: FuelType(id=id, abbrev="A", fuel_type_code="A", description="A")
