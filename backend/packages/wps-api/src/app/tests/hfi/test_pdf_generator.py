...
from wps_shared.schemas.hfi_calc import HFIResultResponse, FireCentre
from wps_shared.schemas.shared import FuelType

from app.hfi.pdf_generator import generate_html, generate_pdf, get_pdf_filename
...
def test_generate_html():
    ...
    html_string, _ = generate_html(
        HFIResultResponse(**result),
        fire_centres,
        "wps",
    ...
    assert (
        hashlib.sha256(html_string.encode()).hexdigest()
...
def test_generate_pdf():
    ...
    (result, fire_centres, fuel_types) = generate_test_input()
    pdf_bytes, pdf_filename = generate_pdf(
        HFIResultResponse(**result),
        fire_centres,
        "wps",
        datetime.fromisocalendar(2022, 2, 2),
