...
def parse_table_records_to_noon_forecast_response(
    data: List[wps_shared.db.models.forecasts.NoonForecast],
):
    ...
    for record in data:
        ...
        noon_forecast_value = NoonForecastValue(
            ...
            if math.isnan(record.relative_humidity)
            else record.relative_humidity,
    ...
    for key, value in noon_forecasts.items():
        noon_forecast = NoonForecast(station_code=key, values=value)
        values.append(noon_forecast)
    return NoonForecastResponse(noon_forecasts=values)


def fetch_noon_forecasts(
    stations: StationCodeList, start_date: datetime, end_date: datetime
) -> NoonForecastResponse:
    ...
    logger.debug(
        "Querying noon forecasts for stations %s from %s to %s", stations, start_date, end_date
    )
    with wps_shared.db.database.get_read_session_scope() as session:
        forecasts = query_noon_forecast_records(session, stations, start_date, end_date)
