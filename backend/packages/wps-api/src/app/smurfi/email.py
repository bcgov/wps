...
def build_spot_forecast_email(spot_forecast, spot_detail_url: str) -> tuple[str, str]:
    ...
    fire_numbers = ", ".join(sr.fire_number) if sr.fire_number else "N/A"
    subject = f"Spot Forecast Update: Fire {fire_numbers}"

    descriptive_rows = ""
    for dw in spot_forecast.descriptive_weather:
        descriptive_rows += (
            f"<tr><td>{dw.period}</td><td>{dw.conditions or ''}</td>"
    ...
    for tw in spot_forecast.tabular_weather:
        ...
        tabular_rows += (
            ...
            f"<td>{tw.relative_humidity}%</td><td>{tw.wind or ''}</td>"
...
<p><strong>Location:</strong> {sr.geographic_description}</p>
...
<p><a href="{spot_detail_url}">View Spot Forecast</a></p>
</body></html>
"""
