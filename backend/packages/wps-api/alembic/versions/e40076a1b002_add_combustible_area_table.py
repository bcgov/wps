...
def create_combustible_area(
    session: Session, advisory_shape_id: int, combustible_area: float, fuel_type_raster_id: int
):
...
def get_current_fuel_type_raster(session: Session):
    ...
    stmt = (
        ...
        .where(fuel_type_raster_table.c.object_store_path.contains(fuel_raster_name))
