...
advisory_elevation_stats = sa.Table(
...
def upgrade():
    ...
    session.execute(
        high_hfi_table.delete().where(high_hfi_table.c.advisory_shape_id.in_(zone_shape_ids))
    )
    session.execute(
        advisory_elevation_stats.delete().where(
            advisory_elevation_stats.c.advisory_shape_id.in_(zone_shape_ids)
        )
    )
