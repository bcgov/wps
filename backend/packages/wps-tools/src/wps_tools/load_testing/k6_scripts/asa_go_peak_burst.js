...
// SOURCE IP. This script's target rate is calibrated to that per-IP limit, not to the full
// 300,000 req/min peak (18,000,000 / 60) -- reaching the full peak depends entirely on
// run_k6_test.py's --task-count fanning this SAME script out across that many parallel
// Fargate tasks (each gets its own source IP), not on anything in this file:
//
//   300,000 req/min target / 100 req/min per task ~= 3,000 Fargate tasks needed
//
// 3,000 concurrent tasks is an extreme scale -- check AWS Fargate service quotas and the
// DLT CloudFormation template's own concurrency limits before running this for real, and be
// aware of the cost (3,000 tasks x run duration, plus data transfer). Start with a small
// --task-count to validate the script itself before attempting the full watermark.
//
...
// limit); override with TARGET_RPS to try a different per-task rate, e.g. for a smaller
// smoke run before committing to a large --task-count.
