...
- **VPC creation blocked by your account's Service Control Policy** (e.g. BC
  Gov's landing zone denies `ec2:CreateVpc`/`ec2:CreateInternetGateway`).
  Deploy fails with `ROLLBACK_COMPLETE`. Fix: tear down, then redeploy with
  `--existing-vpc-id <vpc> --existing-subnet-a <subnet> --existing-subnet-b <subnet>`
