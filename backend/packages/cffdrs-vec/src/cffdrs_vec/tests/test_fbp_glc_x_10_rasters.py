...
test_fbp_glc_x_10.py/test_fbp_glc_x_10_paper.py, but sourced from the GeoTIFF fixtures under
fixtures/glc_x_10/ and fixtures/glc_x_10_paper/ (generated by generate_glc_x_10_rasters.py)
instead of the CSVs directly - proving cffdrs_vec.fbp's vectorized functions also work correctly
when driven by real GDAL raster reads (float32 storage, WPSDataset's nodata handling) rather than
plain float64 arrays parsed straight from CSV.
...
test_fbp_glc_x_10_paper.py for why) sits next to the R-sourced value instead of two files that
have to be compared side by side. Each test's parametrize table only carries the fields that
test actually needs. See https://github.com/bcgov/wps/issues/4893.
"""
