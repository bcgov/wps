...
def test_calculate_combustible_area_by_fire_zone_groups_raster_values(tmp_path: Path):
    ...
    fuel_raster = WPSDataset.from_array(
        np.array([[1, 99], [2, 2]], dtype=np.uint8),
        georeference,
    ...
    result = calculate_combustible_area_by_fire_zone(fuel_path, zone_path)

    assert result == {1: 4, 2: 4}
