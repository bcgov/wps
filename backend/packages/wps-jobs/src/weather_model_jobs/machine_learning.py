...
class RegressionModels:
    ...
    keys = ("temperature_wrapper", "relative_humidity_wrapper", "wind_speed_wrapper")
...
class StationMachineLearning:
    ...
    def _add_sample_to_collection(
        ...
        for model_key, sample_key in zip(SCALAR_MODEL_VALUE_KEYS, SAMPLE_VALUE_KEYS):
            ...
            if model_value is not None:
                ...
                sample_value.add_sample(
                    model_value, actual_value, actual.weather_date, model_key, sample_key
                )
            else:
    ...
    def _collect_data(self, data: list[tuple[HourlyActual, Optional[ModelRunPrediction]]]):
        ...
        for actual, prediction in data:
            if prev_actual != actual and prediction is not None:
                if (
                    ...
                    self._add_sample_to_collection(noon_prediction, prev_actual, sample_collection)
    ...
    def _learn_models(self, start_date: datetime):
        ...
        # materialize so we can iterate over it multiple times
        query_results: list[tuple[HourlyActual, Optional[ModelRunPrediction]]] = list(query)

        # Use the same results for both classic and v2 regression
        sample_collection = self._collect_data(query_results)
        ...
        for sample_key, wrapper_key in zip(SAMPLE_VALUE_KEYS, RegressionModels.keys):
            sample = getattr(sample_collection, sample_key)
            for hour in sample.hours():
                regression_model = getattr(self.regression_models[hour], wrapper_key)
                regression_model.model.fit(sample.np_x(hour).reshape(-1, 1), sample.np_y(hour))
        ...
        self.regression_models_v2.collect_data(query_results)
    ...
    def unwrap_np_float(self, value):
        return value.item() if isinstance(value, np.float64) else value
    ...
    def predict_temperature(self, model_temperature: float, timestamp: datetime):
        ...
        if self.regression_models[hour].temperature_wrapper.good_model:
            return self.unwrap_np_float(
                self.regression_models[hour].temperature_wrapper.model.predict(
                    [[model_temperature]]
                )[0]
            )
        return None
    ...
    def predict_rh(self, model_rh: float, timestamp: datetime):
        ...
        hour = timestamp.hour
        if (
            self.regression_models[hour].relative_humidity_wrapper.good_model
            and model_rh is not None
        ):
            predicted_rh = self.regression_models[hour].relative_humidity_wrapper.model.predict(
                [[model_rh]]
            )[0]
            # in the real world the RH value can't be negative. Sometimes linear regression returns negative value, so assume 0
            return self.unwrap_np_float(min(max(0, predicted_rh), 100))
        return None
    ...
    def predict_wind_speed(self, model_wind_speed: float, timestamp: datetime):
        ...
        hour = timestamp.hour
        if (
            self.regression_models[hour].wind_speed_wrapper.good_model
            and model_wind_speed is not None
        ):
            predicted_wind_speed = self.regression_models[hour].wind_speed_wrapper.model.predict(
                [[model_wind_speed]]
            )[0]
            # in the real world the wind speed can't be negative. Sometimes linear regression returns negative value, so assume 0
            return self.unwrap_np_float(max(0, predicted_wind_speed))
        return None
    ...
    def predict_wind_direction(
        ...
        predicted_wind_dir_deg = calculate_wind_dir_from_u_v(
            predicted_wind_dir[0], predicted_wind_dir[1]
        )
        return self.unwrap_np_float(predicted_wind_dir_deg)
    ...
    def predict_precipitation(self, model_precipitation: float, timestamp: datetime):
        ...
        if math.isnan(model_precipitation):
            ...
            return None
        hour = timestamp.hour
        predicted_precip_24h = self.regression_models_v2._precip_model.predict(
            hour, [[model_precipitation]]
        )
        if predicted_precip_24h is None or len(predicted_precip_24h) == 0:
            # No data to return
            return None
        return self.unwrap_np_float(max(0, predicted_precip_24h[0]))
