...
def test_zero_elevation_identity():
    ...
    out = compute_sea_level_values(temps, elevs, LAPSE_RATE)
    assert_allclose(out, temps, atol=0)

    # round-trip with actual temps:
    rt = compute_adjusted_values(out, elevs, LAPSE_RATE)
...
def test_positive_elevation_cools_when_applying_to_terrain():
    ...
    out = compute_adjusted_values(sea, elev, LAPSE_RATE)
...
def test_broadcasting_shapes():
    ...
    out = compute_adjusted_values(temps, elevs, LAPSE_RATE)
...
def test_dtype_is_float32_even_if_inputs_float64():
    ...
    sea = compute_sea_level_values(temps, elevs, LAPSE_RATE)
    assert sea.dtype == np.float32

    act = compute_adjusted_values(sea, elevs.astype(np.float32), LAPSE_RATE)
...
def test_round_trip_sea_then_actual_returns_original():
    ...
    sea = compute_sea_level_values(temps, elevs, LAPSE_RATE)
    back = compute_adjusted_values(sea, elevs, LAPSE_RATE)
