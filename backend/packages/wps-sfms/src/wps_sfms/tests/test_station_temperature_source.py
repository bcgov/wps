...
def test_zero_elevation_identity():
    ...
    out = LapseRateAdjustedSource.compute_sea_level_values(temps, elevs, LAPSE_RATE)
    assert_allclose(out, temps, atol=0)

    # round-trip with actual temps:
    rt = LapseRateAdjustedSource.compute_adjusted_values(out, elevs, LAPSE_RATE)
...
def test_positive_elevation_cools_when_applying_to_terrain():
    ...
    out = LapseRateAdjustedSource.compute_adjusted_values(sea, elev, LAPSE_RATE)
...
def test_broadcasting_shapes():
    ...
    out = LapseRateAdjustedSource.compute_adjusted_values(temps, elevs, LAPSE_RATE)
...
def test_dtype_is_float32_even_if_inputs_float64():
    ...
    sea = LapseRateAdjustedSource.compute_sea_level_values(temps, elevs, LAPSE_RATE)
    assert sea.dtype == np.float32

    act = LapseRateAdjustedSource.compute_adjusted_values(sea, elevs.astype(np.float32), LAPSE_RATE)
...
def test_round_trip_sea_then_actual_returns_original():
    ...
    sea = LapseRateAdjustedSource.compute_sea_level_values(temps, elevs, LAPSE_RATE)
    back = LapseRateAdjustedSource.compute_adjusted_values(sea, elevs, LAPSE_RATE)
...
def test_round_trip_property(temps, elevs, lapse):
    ...
    sea = LapseRateAdjustedSource.compute_sea_level_values(temps, elevs, lapse)
