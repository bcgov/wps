...
def patch_mask_dataset(mocker, dataset: WPSDataset) -> None:
    @contextmanager
    def dataset_context():
        yield dataset

    mocker.patch("wps_sfms.raster_output.open_bc_mask_dataset", side_effect=dataset_context)


def test_applies_zero_and_nodata_mask_pixels_without_mutating_values(mocker):
    ...
    patch_mask_dataset(mocker, mask)
...
def test_rejects_output_array_shape_that_does_not_match_reference(mocker):
    reference = make_dataset(np.ones((2, 2), dtype=np.float32))
...
def test_rejects_mask_grid_that_does_not_match_reference(mocker):
    reference = make_dataset(np.ones((2, 2), dtype=np.float32))
