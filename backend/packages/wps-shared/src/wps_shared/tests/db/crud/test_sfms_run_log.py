...
async def test_save_sfms_run_log(async_session):
    ...
    assert saved.job_name == "sfms_daily_actuals"
...
async def test_multiple_run_logs(async_session):
    """Test inserting multiple run log records returns unique ids."""
    record1 = SFMSRunLog(
        job_name="sfms_daily_actuals",
        target_date=test_target_date,
        started_at=test_started_at,
    ...
    record2 = SFMSRunLog(
        ...
        started_at=datetime(2025, 7, 16, 20, 30, 0, tzinfo=timezone.utc),
        status="running",
    )
