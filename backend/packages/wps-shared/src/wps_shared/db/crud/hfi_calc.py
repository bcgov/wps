...
def get_fire_weather_stations(
    session: Session,
) -> List[Row[Tuple[PlanningWeatherStation, FuelType, PlanningArea, FireCentre]]]:
    """Get all PlanningWeatherStation with joined FuelType, PlanningArea and FireCentre
    for the provided list of station_codes."""
    return (
        session.query(PlanningWeatherStation, FuelType, PlanningArea, FireCentre)
...
def get_fire_centre_stations(session: Session, fire_centre_id: int) -> CursorResult:
    ...
    return (
        session.query(PlanningWeatherStation, FuelType)
        .join(PlanningArea, PlanningArea.id == PlanningWeatherStation.planning_area_id)
        .join(FuelType, FuelType.id == PlanningWeatherStation.fuel_type_id)
        .filter(PlanningWeatherStation.is_deleted == False)
...
def get_planning_weather_stations(
    session: Session, fire_centre_id: int
) -> List[PlanningWeatherStation]:
...
def get_most_recent_updated_hfi_request(
    session: Session, fire_centre_id: int, date_range: DateRange
) -> HFIRequest:
    """Get the most recently updated hfi request for a fire centre"""
    query = (
        session.query(HFIRequest)
...
def get_most_recent_updated_hfi_request_for_current_date(
    session: Session, fire_centre_id: int
) -> HFIRequest:
...
def store_hfi_request(session: Session, hfi_result_request: HFIResultRequest, username: str):
    ...
    if previous_latest_request is not None:
        ...
        for latest_hfi_ready_record in latest_hfi_ready_records:
            updated_hfi_ready_records.append(
                HFIReady(
                    hfi_request_id=latest_hfi_request_id,
...
def get_latest_hfi_ready_records(session: Session, hfi_request_id: int) -> List[HFIReady]:
    ...
    return session.query(HFIReady).filter(HFIReady.hfi_request_id == hfi_request_id).all()


def get_last_station_in_planning_area(
    session: Session, planning_area_id: int
) -> PlanningWeatherStation:
...
def get_planning_areas(session: Session, fire_centre_id: int):
    ...
    return session.query(PlanningArea).filter(PlanningArea.fire_centre_id == fire_centre_id).all()
...
def get_stations_for_affected_planning_areas(
    session: Session, request: HFIAdminStationUpdateRequest
):
    removed_planning_area_ids = [
        remove_request.planning_area_id for remove_request in request.removed
    ]
...
def unready_planning_areas(session: Session, fire_centre_id: int, username: str, planning_area_ids):
    now = get_utc_now()
    query = (
        update(HFIReady)
        .values(
...
def save_hfi_stations(session: Session, stations_to_save: List[PlanningWeatherStation]):
...
def toggle_ready(
    ...
    ready_state: HFIReady = (
        session.query(HFIReady)
...
def get_fire_centre_fire_start_ranges(session: Session, fire_centre_id: id) -> CursorResult:
    ...
    return (
        session.query(FireStartRange)
        .join(
            FireCentreFireStartRange,
            FireCentreFireStartRange.fire_start_range_id == FireStartRange.id,
        )
