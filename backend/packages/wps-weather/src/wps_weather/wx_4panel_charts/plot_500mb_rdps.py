...
CFG_500 = {
  ...
  #  "extent": [-150.0, -100.0, 35.0, 70.0],  # lon_min, lon_max, lat_min, lat_max
...
def draw_vort_symbol(ax, lon, lat, sign, value, pc, lon_min, lon_max, lat_min, lat_max, cfg):
    ...
    txt = ax.text(lon, lat + 0.4, sign, transform=pc, ha="center", va="center",
                  fontsize=fs_s, color="black", zorder=7)
    ...
    val_txt = ax.text(lon, lat - 0.3, f"{int(abs(value))}", transform=pc, ha="center", va="center",
                      fontsize=fs_v, color="black", zorder=7)
...
def _tri_projected_safe(lon2, lat2, fld2, extent, proj, pc,
    ...
    Prevents 0–360 seam artifacts and long skinny triangles.
...
def plot_500hpa(cfg=None, ax=None):
    ...
    lon2 = normalize_lon_to_extent(ds_z500["longitude"].values, extent)
    ...
    # --- Height contours (projected safe triangulation) ---
    ...
    ax.tricontour(
        ...
        colors="black",
        linewidths=1.0,
        transform=proj,   # IMPORTANT (x/y are projected)
        zorder=5
    )
    ax.tricontour(
        triZ, vZ,
    ...
    # --- Vorticity contours (projected safe triangulation) ---
    triV, vV = _tri_projected_safe(
        lon2, lat2, V, extent, proj, pc,
        stride=stride, clip=clip, pad_deg=pad_deg, max_edge_m=max_edge_m
    )
