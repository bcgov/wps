...
CFG_MSLP_RDPS = {
    ...
    "tri_clip_pad_deg": 10.0,
    "tri_max_edge_m": 350000.0,   # 250k–450k; tune if needed
...
def _tri_projected_safe(lon2, lat2, fld2, extent, proj, pc,
                        stride=2, clip=True, pad_deg=10.0, max_edge_m=350000.0):
    """
    Triangulate in projected x/y (meters) and mask triangles with long edges.
    Works for RDPS (-180..180) and GEM regional (0..360) once lon is normalized.
    """
...
def plot_mslp_thickness_rdps(cfg=None, ax=None, ds_msl=None, ds_thk=None):
    ...
    lon2 = normalize_lon_to_extent(ds_msl["longitude"].values, extent)
    # Units
    ...
    ax.add_feature(provinces, edgecolor="black", linewidth=cfg["province_lw"], zorder=12)
    ...
    pad_deg = float(cfg.get("tri_clip_pad_deg", 10.0))
    max_edge_m = float(cfg.get("tri_max_edge_m", 350000.0))
    
    # --- MSLP contours (projected-safe triangulation) ---
    msl_levels = np.array(cfg["mslp_levels"], dtype=float)
    triZ, vZ = _tri_projected_safe(
        lon2, lat2, Z, extent, proj, pc,
    ...
    ax.tricontour(
        ...
        transform=proj,   # IMPORTANT (tri is in projected coords)
        zorder=5
    )
   ...
   # --- Thickness hatched band + thickness contours (projected-safe) ---
    triT, vT = _tri_projected_safe(
        lon2, lat2, T, extent, proj, pc,
        stride=stride, clip=clip, pad_deg=pad_deg, max_edge_m=max_edge_m
    )
