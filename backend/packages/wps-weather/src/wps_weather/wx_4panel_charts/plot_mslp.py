...
# Main plotter
# --------------------------------------------------
def plot_mslp_thickness(cfg=None, ax=None, ds_msl=None, ds_thk=None):
    if cfg is None:
        cfg = CFG_MSLP
    ...
    # --- IMPORTANT FIX: add cyclic point to remove dateline seam ---
    Z, lon_plot_cyc = add_cyclic_point(Z, coord=lon_plot)
    T, _            = add_cyclic_point(T, coord=lon_plot)
    ...
    for lev in msl_levels:
        ...
        txt = ax.text(
            label_lon, lat_lab, f"{int(lev)}",
            transform=pc, ha="center", va="center",
            fontsize=cfg["mslp_label_fontsize"], fontweight="bold", color="white",
            bbox=dict(boxstyle="square,pad=0.1", facecolor="black",
                      edgecolor="black", linewidth=0.7),
            zorder=20
            )
    ...
    for lev in thk_levels:
        ...
        ax.text(
            ...
            transform=pc, ha="center", va="center",
            fontsize=cfg["thk_label_fontsize"], fontweight="bold",color="black",
