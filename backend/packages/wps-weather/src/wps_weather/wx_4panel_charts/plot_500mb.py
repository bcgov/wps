...
CFG_500 = {
 ...
 #   "vort_index": "rv500.idx",
    ...
    # --- Vorticity ---
    "vort_levels": list(np.arange(4, 32, 8)),
    "vort_colors": [
...
def plot_500hpa(cfg=None, ax=None):
    ...
    # --- IMPORTANT FIX (same idea as precip): make fields cyclic in longitude ---
    Z, lon_plot_cyc = add_cyclic_point(Z, coord=lon_plot)
    V, _            = add_cyclic_point(V, coord=lon_plot)
...
# )
    ...
    for lev in hlev:
        ...
        #         zorder=20)
        txt = ax.text(
        cfg["height_label_lon"], lat_lab, f"{int(lev)}",
        transform=pc,
        ha="center", va="center",
