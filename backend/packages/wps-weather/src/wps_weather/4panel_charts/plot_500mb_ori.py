...
def plot_500hpa(cfg=None, ax=None):
    ...
    # --- Load data ---
    ds_z500 = open_ds(ROOT / cfg["z500_grib"], cfg["z500_index"])
    ds_vort = open_ds(ROOT / cfg["vort_grib"], cfg["vort_index"])
    ...
    ax.contour(lon_plot, lats, Z, levels=hlev, colors="black", linewidths=1.0,
               transform=pc, zorder=5)
    ax.contour(lon_plot, lats, Z, levels=cfg["height_highlight"], colors="black", linewidths=2.0,
    ...
    if cfg["show_vort_symbols"]:
        ...
        neg_final = further_thin_vort(neg[0][neg_k], neg[1][neg_k], lat_idx, lon_idx, lats, lon_plot, V_sub,
                                      min_deg=cfg["vort_deg_min_dist"], positive=False)
