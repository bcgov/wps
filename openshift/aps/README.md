...
| ---------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gw-313f6` | Production APS portal | Real production releases only (`production.yml`) — both Kong gateway-config publishing (`gwa publish-gateway`) and, for SFMS FWI, Dataset/Product API Directory publishing (`gwa apply`). Credentials: `secrets.GWA_ACCT_ID` / `secrets.GWA_ACCT_SECRET`. Service account needs `GatewayConfig.Publish` (+ `Content.Publish` for SFMS FWI's dataset/product).                                                                    |
